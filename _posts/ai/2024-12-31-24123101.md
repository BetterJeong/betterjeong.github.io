...
| 문자의 무작위 증강 | `nac.RandomCharAug(action)` | 삽입(insert), 대체(substitute), 교체(swap), 삭제(delete) |
| 문자의 타입 증강 | `naw.RandomWordAug(action)` | 대체(substitute), 교체(swap), 삭제(delete), 자르기(crop) |
| 문자의 타입 증강 | `nac.RandomCharAug(action)` | 워드넷(wordnet), 의역 데이터베이스(ppdb) |
| 에러없이 증강 | `naw.SynonymAug(aug_src)`  | 대체(substitute)                       |
| 최초 오류 증강 | `naw.ReservedAug(reserved_tokens)` | 대체(substitute)                       |
| 상황별 타입 업데이트 증강 | `naw.SpellingAug()`      | 삽입(insert), 대체(substitute)         |
| 언어없어 증강  | `naw.BackTranslationAug(from_model_name, to_model_name)` | 역번역       |
