...
내가 블로그 호스팅을 제공하는 곳으로 GitHub를 선택한 이유는 단순히 호기심 때문이었다. 처음에는 1학년 과정으로 HTML을 배우고 있으니, HTML5와 CSS, JS를 이용한 개인 홈페이지를 만들고자 했다. 호스팅을 할당 받으려면 유료로 해야 해서 혼자 볼 수 있게라도 해보려고 했다. 그렇게 HTML로 블로그 만드는 법에 대해 구글링을 하다가 우연히 Jekyll로 만들어진 블로그를 발견했다. 